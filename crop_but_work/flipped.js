...
chrome.runtime.onMessage.addListener((message) => {
    if (message.action === 'processImage')
    {
        ...
        image.onload = () => {
            ...
            // Set flipped canvas size
            flippedCanvas.width = width;
            flippedCanvas.height = height;

            // Flip the context horizontally and draw the cropped image
            flippedContext.translate(width, 0); // Move the origin to the right edge
            flippedContext.scale(-1, 1); // Flip horizontally

            // Draw the cropped image on the flipped context
            flippedContext.drawImage(canvas, 0, 0);

            // Now you can use the flippedCanvas or display it
            // For example, you can add it to the body or another element
            document.body.appendChild(flippedCanvas);

            // Optionally, position the flipped image at the original crop location
            flippedCanvas.style.position = 'absolute';
            flippedCanvas.style.left = `${abs_startX}px`;
            flippedCanvas.style.top = `${abs_startY}px`;

            const handleClickOutside = (event) => {
                if (!flippedCanvas.contains(event.target))
                {
